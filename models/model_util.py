...
def todevice(net,gpu_id):
    """Legacy helper. Safe on machines without CUDA; prefer DeviceManager."""
    try:
        import torch
        has_cuda = torch.cuda.is_available()
    except Exception:
        has_cuda = False
    if gpu_id != '-1' and len(gpu_id) == 1 and has_cuda:
        net.cuda()
    elif gpu_id != '-1' and len(gpu_id) > 1 and has_cuda:
...
def normalize_vgg_input(tensor):
    """Convert RGB tensors from generator [-1, 1] to ImageNet VGG space."""
    value = ((tensor + 1.0) * 0.5).clamp(0.0, 1.0)
...
class VGGLoss(nn.Module):
    ...
    def forward(self, x, y):
        x_vgg = self.vgg(normalize_vgg_input(x))
        y_vgg = self.vgg(normalize_vgg_input(y))
        loss = 0
